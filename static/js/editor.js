class Editor {
    ...
    setActiveClass(id) {
        ...
        if (activeObjects.length > 0) {
            ...
            this.canvas.requestRenderAll();
            // Update UI
            // onSelect expects {selected: [obj]} or we can manually refresh inspector?
            // onSelect actually clears inspector if multiple are selected because of `e.selected[0]`.
            // But if we want to confirm, we can just let it be. 
            // Or better, trigger onSelect again if single.
            if (activeObjects.length === 1) {
                this.onSelect({ selected: [activeObjects[0]] });
            }
        }
