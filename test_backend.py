...
CLASSES_FILE = os.path.join(TEST_DIR, "classes.txt")
...
with app.app_context():
    ...
    # 2. Create Project
    p = Project(name="Test Project", root_path=IMAGES_DIR, classes_path=CLASSES_FILE)
    db.session.add(p)
    db.session.commit()
    ...
    res = utils.export_dataset([p.id], 0.8)
